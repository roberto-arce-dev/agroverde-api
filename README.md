...
## 📸 Crear ProductoAgricolas con Imágenes

### ⚠️ IMPORTANTE: La imagen es OPCIONAL

La imagen **NO es requerida** al crear un productoagricola. Puedes:
- ✅ Crear el productoagricola SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el ProductoAgricola sin imagen

```bash
POST /api/productoagricola
Authorization: Bearer {{token}}
Content-Type: application/json

{
  "nombre": "Lechugas Hidropónicas",
  "descripcion": "Lechugas cultivadas en sistema hidropónico",
  "precio": 3.5,
  "unidad": "unidad",
  "stock": 50
}
...
**💡 Nota:** Guarda el `_id` del productoagricola creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al ProductoAgricola

```bash
POST /api/productoagricola/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
8. **Copiar** el `_id` del productoagricola creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload ProductoAgricola Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3011/api/productoagricola/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu productoagricola ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3011/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3011/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
# 2. Crear ProductoAgricola
PRODUCTO_ID=$(curl -X POST http://localhost:3011/api/productoagricola \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "nombre": "Lechugas Hidropónicas",   "descripcion": "Lechugas cultivadas en sistema hidropónico",   "precio": 3.5,   "unidad": "unidad",   "stock": 50 }' \
  | jq -r '.data._id')

echo "ProductoAgricola creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3011/api/productoagricola/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3011/uploads/`
...
```bash
curl -X GET http://localhost:3011/api/cliente-profile/me \
...
```bash
curl -X PUT http://localhost:3011/api/cliente-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `agroverde-api.postman_collection.json`
...
├── agroverde-api.postman_collection.json
└── package.json
```
